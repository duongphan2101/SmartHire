  ...
  const handleSave = async () => {
    if (!draft.trim()) {
      Swal.fire({
        icon: "error",
        title: "Lỗi!",
        text: "Nội dung điều khoản không được để trống.",
        confirmButtonColor: "#ef4444",
      });
      return;
    }

    const result = await updateTerms("user", draft);

    if (result.success) {
      Swal.fire({
        icon: "success",
        title: "Đã lưu thành công!",
        text: "Điều khoản người dùng đã được cập nhật.",
  ...
  return (
    <div className="terms-admin-container">
      <div className="terms-admin-header">
        <h2>📜 Điều khoản dành cho Người dùng</h2>
      </div>
      ...
      <div className="terms-admin-content">
        {isLoading ? (
          ...
          <pre className="terms-admin-text">
            {termsText || "Không tìm thấy nội dung điều khoản người dùng."}
          </pre>
        )}
      </div>
