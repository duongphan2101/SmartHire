...
const JobPost = ({ onOpenChatRequest }: JobPostProps) => {
  ...
  const jobs = Array.isArray(allJobs)
    ? allJobs.filter(
        (job) => String(job?.createBy?._id) === String(currentUserId)
      )
    : [];

  const handleAddClick = () => {
    // 1. Kiểm tra trạng thái Tạm khóa (Suspended)
    if (department && department.status === "Suspended") {
      MySwal.fire({
        title: "Tạo tin thất bại ⛔",
        html: `
                Công ty của bạn hiện đang ở trạng thái 
                <b style="color:red">Tạm khóa</b> bởi Quản trị viên.<br>
                Bạn không thể đăng tin tuyển dụng mới.<br>
    ...
    // 2. Kiểm tra trạng thái Chờ duyệt (Pending)
    if (department && department.status === "Pending") {
      MySwal.fire({
        icon: "warning",
  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filteredResults = data.filter(
        (job: any) => job.createBy._id === currentUserId
      );
  ...
  if (error)
    return (
      <div className="text-2xl" style={{ padding: 20 }}>
        <Empty
          image={Empty.PRESENTED_IMAGE_SIMPLE}
          description="Không có bài đăng!"
        />
      </div>
    );
            ...
            <div className="job-card" key={job._id}>
              ...
              <div className="job-footer">
                <div className="flex gap-3 flex-wrap">
                  ...
                  <span
                    style={{ fontSize: 13 }}
                    className="job-address text-gray-400"
                  >
                    {job.address}
                  </span>
                </div>
